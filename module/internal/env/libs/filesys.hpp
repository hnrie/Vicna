...
namespace Filesystem
{
    ...
    inline int getcustomasset(lua_State* L) {
        ...
        try {
            ...
            auto customAssetsDir = std::filesystem::current_path() / "ExtraContent" / "leafy";
            ...
            std::filesystem::copy_file(FullPath, customAssetsFile, std::filesystem::copy_options::overwrite_existing);

            std::string Final = "rbxasset://leafy/" + customAssetsFile.filename().string();
    ...
    inline void RegisterLibrary(lua_State* L)
    {
        ...
        Utils::RegisterAliases(L, getcustomasset, {
            "getcustomasset", "get_custom_asset", "getsynasset", "get_syn_asset"
            });

        Utils::RegisterAliases(L, fs_getworkspace, {
            "getworkspace", "get_workspace", "getworkspacepath", "get_workspace_path"
            });
    }
}