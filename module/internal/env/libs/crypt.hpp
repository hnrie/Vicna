...
inline std::map<std::string, HashModes> HashTranslationMap = {
    ...
    {"sha224", HASH_SHA224},     {"sha256", HASH_SHA256},
    {"sha384", HASH_SHA384},     {"sha512", HASH_SHA512},
    {"sha3-224", HASH_SHA3_224}, {"sha3_224", HASH_SHA3_224},
    {"sha3-256", HASH_SHA3_256}, {"sha3_256", HASH_SHA3_256},
    {"sha3-384", HASH_SHA3_384}, {"sha3_384", HASH_SHA3_384},
    {"sha3-512", HASH_SHA3_512}, {"sha3_512", HASH_SHA3_512},
};
...
std::vector<unsigned char> getProtoBytes(const Proto *proto) {
  ...
  for (auto i = 0; i < proto->sizek; i++) {
    ...
    case LUA_TSTRING: {
      ...
      protoBytes.insert(
          ...
          reinterpret_cast<const unsigned char *>(currentConstantString->data) +
              currentConstantString->len);
      break;
    }
    case LUA_TNUMBER: {
      const lua_Number *n = &currentConstant->value.n;
      protoBytes.insert(
          protoBytes.end(), reinterpret_cast<const unsigned char *>(n),
    ...
    case LUA_TBOOLEAN:
      ...
      break;
    case LUA_TVECTOR:
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[0] & 0xff000000);
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[0] & 0x00ff0000);
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[0] & 0x0000ff00);
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[0] & 0x000000ff);
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[1] & 0xff000000);
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[1] & 0x00ff0000);
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[1] & 0x0000ff00);
      protoBytes.push_back(
          reinterpret_cast<int *>(currentConstant->value.v)[1] & 0x000000ff);
    ...
    case LUA_TFUNCTION: {
      ...
      std::vector<unsigned char> functionBytes =
          getProtoBytes(constantFunction->l.p);
      protoBytes.insert(protoBytes.end(), functionBytes.data(),
  ...
  protoBytes.insert(protoBytes.end(),
                    reinterpret_cast<const unsigned char *>(proto->code),
                    reinterpret_cast<const unsigned char *>(proto->code) +
                        proto->sizecode * sizeof(Instruction));
  return protoBytes;
}
