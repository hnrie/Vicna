...
static bool getgc_visit(void *context, lua_Page *page, GCObject *gco) {
  ...
  if (tt == LUA_TFUNCTION || tt == LUA_TUSERDATA ||
      (ctx->include_tables && tt == LUA_TTABLE)) {
    TValue obj;
    switch (tt) {
    case LUA_TFUNCTION:
...
int getrenv(lua_State *L) {
  ...
  lua_State *roblox_state =
      TaskScheduler::GetLuaStateForInstance(ScriptContext);
      ...
      if (lua_pcall(L, 1, 1, 0) == 0 && lua_istable(L, -1)) {
        ...
        while (lua_next(L, -2)) {
          if (lua_isuserdata(L, -1)) {
            lua_getfield(L, -1, "ClassName");
            if (lua_isstring(L, -1) &&
                strcmp(lua_tostring(L, -1), "Actor") == 0) {
              lua_pop(L, 1);
              lua_pushvalue(L, -1);
              lua_rawseti(L, 1, idx++);
            } else {
...
int getsenv(lua_State *s) {
  ...
  if (class_name == "LocalScript" || class_name == "Script") {
    auto node = *reinterpret_cast<uintptr_t *>(
        script + Offsets::Scripts::weak_thread_node);
    auto weakref = *reinterpret_cast<uintptr_t *>(
        node + Offsets::Scripts::weak_thread_ref);
    auto liveref = *reinterpret_cast<uintptr_t *>(
        weakref + Offsets::Scripts::weak_thread_ref_live);
    lua_State *liverefthread = *reinterpret_cast<lua_State **>(
        liveref + Offsets::Scripts::weak_thread_ref_live_thread);
    if (!liverefthread) {
      lua_pushnil(s);
      return 1;
    }
...
void SetIdentity(lua_State *l, int lvl) {
  const int64_t NewCapabilities =
      Roblox::GetCapabilities(&lvl) | 0x3FFFFFFFFFFF00i64;
  l->userdata->Identity = lvl;
  l->userdata->Capabilities = NewCapabilities;
  std::uintptr_t identity_struct = Roblox::GetIdentityStruct(
      *reinterpret_cast<std::uintptr_t *>(Offsets::Identity::IdentityPtr));
  if (!identity_struct)
    return;
...
static inline bool tableContainsValue(lua_State *L, int tableIdx,
                                      int valueIdx) {
  lua_pushnil(L);
...
static inline bool tableContainsAllValues(lua_State *L, int sourceTable,
                                          int filterTable) {
  lua_pushnil(L);
...
static inline bool tableContainsAllKeys(lua_State *L, int sourceTable,
                                        int filterTable) {
  lua_pushnil(L);
  while (lua_next(L, filterTable) != 0) {
...
static inline bool tableContainsAllKeyValuePairs(lua_State *L, int sourceTable,
                                                 int filterTable) {
  lua_pushnil(L);
  while (lua_next(L, filterTable) != 0) {
    lua_pushvalue(L, -2);
...
static inline const char *getOptionalStringField(lua_State *L, int tableIdx,
                                                 const char *fieldA,
                                                 const char *fieldB = nullptr) {
...
static inline const char *getOptionalStringField3(lua_State *L, int tableIdx,
                                                  const char *fieldA,
                                                  const char *fieldB,
                                                  const char *fieldC) {
  ...
  if (value)
    return value;
  if (fieldC)
    return getOptionalStringField(L, tableIdx, fieldC, nullptr);
  return nullptr;
}

static inline bool getOptionalBooleanField(lua_State *L, int tableIdx,
                                           const char *fieldA,
                                           const char *fieldB,
                                           bool defaultValue) {
  lua_getfield(L, tableIdx, fieldA);
...
static inline bool getOptionalBooleanField3(lua_State *L, int tableIdx,
                                            const char *fieldA,
                                            const char *fieldB,
                                            const char *fieldC,
                                            bool defaultValue) {
...
static inline int getOptionalTableFieldRef(lua_State *L, int tableIdx,
                                           const char *fieldA,
                                           const char *fieldB = nullptr) {
  int ref = LUA_NOREF;
  lua_getfield(L, tableIdx, fieldA);
...
static inline int getOptionalTableFieldRef3(lua_State *L, int tableIdx,
                                            const char *fieldA,
                                            const char *fieldB,
                                            const char *fieldC = nullptr) {
  int ref = getOptionalTableFieldRef(L, tableIdx, fieldA, fieldB);
  if (ref != LUA_NOREF)
    return ref;
  if (fieldC)
    return getOptionalTableFieldRef(L, tableIdx, fieldC, nullptr);
  return LUA_NOREF;
}

static inline bool functionNameMatches(lua_State *L, int functionIdx,
                                       Closure *cl, const char *expectedName) {
  if (!expectedName)
    return true;
  ...
  if (cl->isC) {
    if (cl->c.debugname)
      rawName = cl->c.debugname;
  } else if (cl->l.p && cl->l.p->debugname) {
    rawName = getstr(cl->l.p->debugname);
  }

  if (rawName) {
    if (strcmp(rawName, expectedName) == 0)
      return true;
    if (strstr(rawName, expectedName) != nullptr)
      return true;
  }
  ...
  if (lua_isfunction(L, -1)) {
    ...
    if (lua_pcall(L, 2, 1, 0) == LUA_OK) {
      if (lua_isstring(L, -1) &&
          strcmp(lua_tostring(L, -1), expectedName) == 0) {
        lua_settop(L, top);
        return true;
      }
      lua_pop(L, 1);
...
inline int filtergc(lua_State *L) {
  ...
  if (isFunction) {
    filterName =
        getOptionalStringField3(L, optionsTable, "Name", "name", "NAME");
    ignoreExecutor =
        getOptionalBooleanField3(L, optionsTable, "IgnoreExecutor",
                                 "ignoreexecutor", "ignoreExecutor", true);
    filterHash =
        getOptionalStringField3(L, optionsTable, "Hash", "hash", "HASH");
    constantsRef = getOptionalTableFieldRef3(L, optionsTable, "Constants",
                                             "constants", "CONSTANTS");
    upvaluesRef = getOptionalTableFieldRef3(L, optionsTable, "Upvalues",
                                            "upvalues", "UPVALUES");
  } else {
    keysRef =
        getOptionalTableFieldRef3(L, optionsTable, "Keys", "keys", "KEYS");
    valuesRef = getOptionalTableFieldRef3(L, optionsTable, "Values", "values",
                                          "VALUES");
    kvpRef = getOptionalTableFieldRef3(L, optionsTable, "KeyValuePairs",
                                       "keyvaluepairs", "keyValuePairs");
    metatableRef = getOptionalTableFieldRef3(L, optionsTable, "Metatable",
                                             "metatable", "METATABLE");
  }
  ...
  while (lua_next(L, gcTableIdx) != 0) {
    ...
    if (isFunction && lua_isfunction(L, objIdx)) {
      ...
      if (filterHash && matches && !cl->isC) {
        ...
        if (lua_isfunction(L, -1)) {
          ...
          if (lua_pcall(L, 1, 1, 0) == LUA_OK) {
            if (!lua_isstring(L, -1) ||
                strcmp(lua_tostring(L, -1), filterHash) != 0) {
              matches = false;
            }
            lua_pop(L, 1);
          } else {
      ...
      if (cl->isC && (filterHash || constantsRef != LUA_NOREF ||
                      upvaluesRef != LUA_NOREF)) {
        matches = false;
      }
    ...
    if (matches) {
      if (returnOne) {
        if (constantsRef != LUA_NOREF)
          lua_unref(L, constantsRef);
        if (upvaluesRef != LUA_NOREF)
          lua_unref(L, upvaluesRef);
        if (keysRef != LUA_NOREF)
          lua_unref(L, keysRef);
        if (valuesRef != LUA_NOREF)
          lua_unref(L, valuesRef);
        if (kvpRef != LUA_NOREF)
          lua_unref(L, kvpRef);
    ...
    if (lua_isthread(L, -1)) {
      ...
      if (thread && thread->userdata) {
        ...
        if (lua_isuserdata(thread, -1)) {
          ...
          if (lua_isuserdata(thread, -1)) {
            uintptr_t parentPtr =
                *reinterpret_cast<uintptr_t *>(lua_touserdata(thread, -1));
...
inline int runonactor(lua_State *L) {
  ...
  if (lua_isfunction(L, 2)) {
    ...
    if (luau_load(actorThread, "=Actor", bytecode.c_str(), bytecode.length(),
                  0) == LUA_OK) {
...
inline int comparefunctions(lua_State *L) {
  ...
  if (!cl1->isC && !cl2->isC) {
    ...
    bool protosMatch =
        (p1->sizecode == p2->sizecode && p1->sizek == p2->sizek &&
         p1->sizep == p2->sizep && p1->nups == p2->nups &&
         p1->numparams == p2->numparams && p1->is_vararg == p2->is_vararg);

    if (protosMatch && p1->sizecode > 0) {
      protosMatch =
          (memcmp(p1->code, p2->code, p1->sizecode * sizeof(Instruction)) == 0);
    }
...
int lz4compress(lua_State *L) {
  ...
  int compressedSize = LZ4_compress_default(data, compressed.data(),
                                            (int)data_len, maxCompressedSize);
...
int lz4decompress(lua_State *L) {
  ...
  int decompressedSize = LZ4_decompress_safe(
      compressed, decompressed.data(), (int)compressed_len, expected_size);
